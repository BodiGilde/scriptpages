...
toon_voortgang() {
    echo "$1" 
    dialog --title "Voortgang" --gauge "Even geduld aub..." 8 50 0 # Toon een voortgangsbalk
}
    ...
    for pakket in "${pakketten[@]}"; do
        if [[ "$pakket" == nodejs* ]]; then
            ...
            if ! dpkg -s "$pakket" >/dev/null 2>&1; then # Controleer of het pakket al is geïnstalleerd
                ...
                sudo apt-get install -y "$pakket" 2>&1 
                dialog --title "Pakket Installatie" --progressbox 20 70 # Toon de installatie output in een dialoogvenster
                if [ $? -eq 0 ]; then
